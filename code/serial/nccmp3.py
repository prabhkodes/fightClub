...
##############################################################################
##
## nccmp3.py: A simple python-based NetCDF 3-way comparison tool. The purpose
##   of this is to show whether files 2 and 3 have more of a difference than
##   files 1 and 2.
##
## The specific purpose is to compare refactored differences against
## presumed bit-level differences (like -O0 and -O3 compiler flags).
## This prints the relative 2-norm of the absolute differences between files
## 1 & 2 and files 2 & 3, as well as the ratio of the relative 2-norms between
## the 2-3 comparison and the 1-2 comparison.
##
## python3 nccmp3.py file1.nc file2.nc file3.nc
##
##############################################################################
##############################################################################
...
if len(sys.argv) < 4:
    print("Usage: python3 "+sys.argv[0]+" file1.nc file2.nc file3.nc")
    sys.exit(1)
...
for v in nc1.variables.keys():
    if v == 'time':
        continue
    # Only compare floats
    if (nc2.variables[v].dtype == np.float64 or
        nc2.variables[v].dtype == np.float32):
        # Grab the variables
        a1 = nc1.variables[v][:]
        a2 = nc2.variables[v][:]
        a3 = nc3.variables[v][:]
        ...
        if (norm12 != 0):
            ...
            # If they're both zero, then just give a ratio of "1",
            # showing they are the same
            if (norm23 == 0):
                normRatio = 1
            # If the 1-2 norm is zero but the 2-3 norm is not, give a
    ...
    # the 1-2 diff.
    # In the future, this should be added as a command line parameter fori
    # the user to choose.
    if normRatio > 0:
        print(v.ljust(20) + 
